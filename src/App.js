...
// 이때 이 컨텍스트를 다른 파일(컴포넌트)에서 불러올 수 있게 export
// context 사용시 지나친 페이지 리렌더링 이슈를 막기 위한
...
function App() {
  ...
  // 세가지 데이터를 인풋으로 받아서 저장
  // 상단의 함수 dispatch를 호출
  // 데이터는 객체로 데이터 저장할 때 타입은 'CREATE'로 함
  // 마지막으로 일기를 저장할 때마다 idRef.current += 1로 id값을
  // 1씩 늘려서 id 데이터가 중복되지 않도록 함

  useEffect(() => {
    // 로컬 스토리지로 부터 diary라는 키 값에 저장해 둔 데이터를
    // 불러와서 rawData 변수에 저장
    // 만약에 rawData가 존재하지 않는다면 setIsDataLoaded를 true로 업데이트 하고 종료
    // 데이터가 존재하면 JSON객체로 복원
      ...
      // 내림차순 정렬이기에 localData[0]는 id 중 가장 큰 값이 됩니다.
      // 그렇게 해서 idRef.current 즉, id의 현재값은
      // 일기 id에서 가장 큰 값에 1 더한 값으로 설정
